...
export function QuizTab({ module }: { module: { id: string; title: string } }) {
    ...
    const handleCheckAnswer = () => {
        ...
        if(selectedAnswer === question.correctAnswer) {
            setFeedback({correct: true, message: "Correct! Excellent job."})
        } else {
            setFeedback({correct: false, message: `Not quite. The correct answer is ${question.correctAnswer}.`})
        }
    }
  ...
  return (
    <div className='max-w-2xl mx-auto'>
        <Card>
            <CardHeader>
                <CardTitle>Topic Quiz: {module.title}</CardTitle>
                <CardDescription>Test your knowledge with a quick question.</CardDescription>
            </CardHeader>
